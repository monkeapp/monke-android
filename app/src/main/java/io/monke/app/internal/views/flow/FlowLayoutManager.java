...
import io.monke.app.internal.views.flow.cache.FlowCacheHelper;
import io.monke.app.internal.views.flow.cache.Line;
...
public class FlowLayoutManager extends RecyclerView.LayoutManager {
    ...
    FlowLayoutHelper layoutHelper;
    FlowCacheHelper cacheHelper;
    @Nullable
    ...
    public void onItemsChanged(RecyclerView recyclerView) {
        ...
        cacheHelper = new FlowCacheHelper(flowLayoutOptions.itemsPerLine, layoutHelper.visibleAreaWidth());
    ...
    public void onAttachedToWindow(final RecyclerView view) {
        ...
        layoutHelper = new FlowLayoutHelper(this, recyclerView);
        cacheHelper = new FlowCacheHelper(flowLayoutOptions.itemsPerLine, layoutHelper.visibleAreaWidth());
    ...
    public FlowLayoutManager singleItemPerLine() {
        ...
        if (layoutHelper != null) {
            cacheHelper = new FlowCacheHelper(1, layoutHelper.visibleAreaWidth());
        }
        return this;
    }
    ...
    public FlowLayoutManager maxItemsPerLine(int itemsPerLine) {
        ...
        if (layoutHelper != null) {
            cacheHelper = new FlowCacheHelper(itemsPerLine, layoutHelper.visibleAreaWidth());
        }
        return this;
    }
    ...
    public FlowLayoutManager removeItemPerLineLimit() {
        ...
        if (layoutHelper != null) {
            cacheHelper = new FlowCacheHelper(FlowLayoutOptions.ITEM_PER_LINE_NO_LIMIT, layoutHelper.visibleAreaWidth());
        }
        return this;
    }
    ...
    private void onPreLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        Point point = layoutHelper.layoutStartPoint(FlowLayoutContext.fromLayoutOptions(flowLayoutOptions));
        ...
        FlowLayoutContext beforeContext = FlowLayoutContext.fromLayoutOptions(flowLayoutOptions);

        // this option use old options alignment & new options line limit to calc items for animation.
        FlowLayoutContext afterContext = FlowLayoutContext.clone(beforeContext);
    ...
    private void onRealLayoutChildren(RecyclerView.Recycler recycler) {
        ...
        boolean newLine;
        Rect rect = new Rect();
        FlowLayoutContext flowLayoutContext = FlowLayoutContext.fromLayoutOptions(flowLayoutOptions);
        for (int i = firstChildAdapterPosition; i < itemCount; i++) {
            View child = recycler.getViewForPosition(i);
            newLine = calcChildLayoutRect(child, x, y, height, flowLayoutContext, rect);
            ...
            if (newLine) {
                ...
                y = lineInfo.y;
                height = rect.height();
                flowLayoutContext.currentLineItemCount = 1;

            } else {
                x = advanceInSameLine(x, rect, flowLayoutContext);
                height = Math.max(height, rect.height());
                flowLayoutContext.currentLineItemCount++;
            }
        }
    }
    ...
    private void addNewLineAtTop(RecyclerView.Recycler recycler) {
        ...
        if (cacheHelper.hasPreviousLineCached(firstItemAdapterPosition)) {
            ...
            while (currentAdapterPosition <= endAdapterPosition) {
                ...
                newline = calcChildLayoutRect(newChild, x, 0, height, flowLayoutContext, rect);
                ...
                if (newline && !firstItem) {
                    ...
                    lineChildren.clear();
                    x = advanceInSameLine(layoutStartPoint().x, rect, flowLayoutContext);
                    height = rect.height();
                    flowLayoutContext.currentLineItemCount = 1;
                } else {
                    x = advanceInSameLine(x, rect, flowLayoutContext);
        ...
        y = bottom - height;
        firstItem = true;
        flowLayoutContext = FlowLayoutContext.fromLayoutOptions(flowLayoutOptions);
        for (int i = 0; i < lineChildren.size(); i++) {
            View childView = lineChildren.get(i);
            newline = calcChildLayoutRect(childView, x, y, height, flowLayoutContext, rect);
            if (newline && firstItem) {
    ...
    private void addNewLineAtBottom(RecyclerView.Recycler recycler) {
        ...
        while (childAdapterPosition < getItemCount()) {
            ...
            newline = calcChildLayoutRect(newChild, x, y, 0, flowLayoutContext, rect);
    ...
    private int getMaxHeightLayoutPositionInLine(int layoutPosition) {
        ...
        FlowLayoutContext flowLayoutContext = FlowLayoutContext.fromLayoutOptions(flowLayoutOptions);
        while (currentIndex >= 0 && !isStartOfLine(currentIndex, flowLayoutContext)) {
        ...
        while (currentIndex < getChildCount() && !isEndOfLine(currentIndex, flowLayoutContext)) {
    ...
    private List<View> getAllViewsInLine(int index) {
        ...
        FlowLayoutContext flowLayoutContext = FlowLayoutContext.fromLayoutOptions(flowLayoutOptions);
        while (nextItemIndex < getChildCount() && !isStartOfLine(nextItemIndex, flowLayoutContext)) {
        ...
        if (adapterPosition > firstChildPosition) {
            ...
            if (lastChildAdapterPosition >= adapterPosition) {
                ...
                while (targetAdapterPosition != adapterPosition) {
                    ...
                    newline = calcChildLayoutRect(nextChild, x, y, height, flowLayoutContext, rect);
                    if (newline) {
                        x = advanceInSameLine(layoutStartPoint().x, rect, flowLayoutContext);
                        y = rect.top;
                        height = rect.height();
    ...
    private boolean calcChildLayoutRect(View child, int x, int y, int lineHeight, Rect rect) {
        return calcChildLayoutRect(child, x, y, lineHeight, FlowLayoutContext.fromLayoutOptions(flowLayoutOptions), rect);
    }

    private boolean calcChildLayoutRect(View child, int x, int y, int lineHeight, FlowLayoutContext FlowLayoutContext, Rect rect) {
        ...
        switch (FlowLayoutContext.layoutOptions.alignment) {
            case RIGHT:
                if (FlowLayoutHelper.shouldStartNewline(x, childWidth, leftVisibleEdge(), rightVisibleEdge(), FlowLayoutContext)) {
                    newLine = true;
    ...
    private Point startNewline(Rect rect) {
        return startNewline(rect, FlowLayoutContext.fromLayoutOptions(flowLayoutOptions));

    }

    private Point startNewline(Rect rect, FlowLayoutContext FlowLayoutContext) {
        switch (FlowLayoutContext.layoutOptions.alignment) {
            case RIGHT:
    ...
    private int advanceInSameLine(int x, Rect rect) {
        return advanceInSameLine(x, rect, FlowLayoutContext.fromLayoutOptions(flowLayoutOptions));
    }

    private int advanceInSameLine(int x, Rect rect, FlowLayoutContext FlowLayoutContext) {
        switch (FlowLayoutContext.layoutOptions.alignment) {
            case RIGHT:
                return x - rect.width();
            case LEFT:
    ...
    private Point layoutStartPoint() {
        return layoutHelper.layoutStartPoint(FlowLayoutContext.fromLayoutOptions(flowLayoutOptions));
    }

    private boolean isStartOfLine(int index) {
        return isStartOfLine(index, FlowLayoutContext.fromLayoutOptions(flowLayoutOptions));
    }

    private boolean isStartOfLine(int index, FlowLayoutContext FlowLayoutContext) {
        if (index == 0) {
            return true;
        } else {
            switch (FlowLayoutContext.layoutOptions.alignment) {
    ...
    private boolean isEndOfLine(int index) {
        return isEndOfLine(index, FlowLayoutContext.fromLayoutOptions(flowLayoutOptions));
    }

    private boolean isEndOfLine(int index, FlowLayoutContext FlowLayoutContext) {
        if (FlowLayoutHelper.hasItemsPerLineLimit(FlowLayoutContext.layoutOptions) && FlowLayoutContext.currentLineItemCount == FlowLayoutContext.layoutOptions.itemsPerLine) {
            return true;
        }
        if (getChildCount() == 0 || index == getChildCount() - 1) {
            return true;
        }
        return isStartOfLine(index + 1, FlowLayoutContext);
    }
